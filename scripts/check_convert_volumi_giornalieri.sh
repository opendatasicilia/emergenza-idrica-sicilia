...
PATH_PDFS_LIST="./risorse/pdfs_list_volumi_giornalieri.txt"
...
#----------------- functions -----------------#
# creo funzione "check limits" che controlla se il numero di richieste ai "n_ai" supera un certo limite
# e in caso mette in pausa il processo per un certo tempo.
# La funzione deve accettare come argomenti il numero di richieste variabile "n_ai" e
# le seguenti costanti: il limite "AI_LIMITS" e il tempo di pausa "AI_SLEEP"
check_limits() {
...
generate_telegram_message() {
   ...
   echo "🔎 Ho trovato ed estratto *nuovi dati* sui *volumi* invasati dalle dighe siciliane!
...
while read -r line; do
   if ! grep -q "$line" $PATH_PDFS_LIST; then
      ...
      # converto il nome del pdf in volumi_giornalieri_YYYY-MM-DD.pdf tramite ai llm
      system_prompt="converti il nome di questo file pdf nel formato volumi_giornalieri_YYYY-MM-DD.pdf. Restituisci in output una sola riga"

      llm_response=$(echo "$line" | llm -m gemini-1.5-flash-latest \
      -s "$system_prompt" -o temperature 0.1) \
      ...
      curl -skL "$URL_HOMEPAGE$line" -o "./risorse/pdf/volumi-giornalieri/$new_filename"
      echo "⬇️  File scaricato e rinominato in $new_filename"
      ...
      -s "$system_prompt" \
      -a "./risorse/pdf/volumi-giornalieri/$new_filename" \
      -o temperature 0.1) \
      ...
      llm -m gemini-1.5-flash-latest -s "$system_prompt" "$prompt" -o json_object 1 > ./risorse/report_extraction_latest.json
      n_ai=$((n_ai+1))
...
if [ -d "./risorse/tmp" ]; then
   ...
   date_filename=$(echo $new_filename | sed 's/^volumi_giornalieri_//; s/\.pdf//')
   cp ./risorse/tmp/$new_filename.csv ./risorse/volumi-giornalieri/$date_filename.csv

   # merge csv data (se ne esistono più di uno)
   # attenzione: può capitare che il csv latest contenga i valori relativi a più date
   mlr --csv cat ./risorse/tmp/*.csv > $PATH_CSV_VOLUMI_GIORNALIERI_LATEST
   ...
   # remove duplicates
   mlr --csv cat $PATH_CSV_VOLUMI_GIORNALIERI_LATEST $PATH_CSV_VOLUMI_GIORNALIERI \
